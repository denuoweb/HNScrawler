...
HSD_CACHE_SIZE_MB="${HSD_CACHE_SIZE_MB:-512}"
...
ExecStart=\$HSD_BIN --prefix $INDEXER_HSD_PREFIX --network main --api-key \\\$HSD_API_KEY --http-host 127.0.0.1 --no-wallet --max-outbound $HSD_MAX_OUTBOUND --log-level $HSD_LOG_LEVEL --cache-size $HSD_CACHE_SIZE_MB --block-cache-size $HSD_BLOCK_CACHE_SIZE_MB --max-files $HSD_MAX_FILES --entry-cache $HSD_ENTRY_CACHE
Restart=always
RestartSec=15
LimitNOFILE=65536
