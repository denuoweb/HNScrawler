...
test -r "$TOPOLOGY_SITE_DIR/data/hns-handoff-groups.json" || {
  echo "HNS handoff cohort export is not readable: $TOPOLOGY_SITE_DIR" >&2
  exit 2
}
...
if ! flock -n 9; then
  echo "a live-directory cycle or priority-index refresh is already running" >&2
  exit 0
fi

cd "$LIVE_REPO_DIR"
.venv/bin/hns-live-directory index-delegations \
  --db "$LIVE_DB" \
  --topology-site "$TOPOLOGY_SITE_DIR" \
  --min-members "$LIVE_DELEGATION_MIN_MEMBERS" \
  --max-members "$LIVE_DELEGATION_MAX_MEMBERS"

exec .venv/bin/hns-live-directory index-handoffs \
  --db "$LIVE_DB" \
  --topology-site "$TOPOLOGY_SITE_DIR" \
  --min-members "$LIVE_HANDOFF_MIN_MEMBERS" \
  --max-members "$LIVE_HANDOFF_MAX_MEMBERS"
