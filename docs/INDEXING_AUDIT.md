...
During export, the generator builds a temporary `export_name_ordinals` table with ordinal, provider, provider type, resource flags, normalized delegated-DNS TLSA evidence, and the derived `compliance_stage` for the exported name set. Nonzero posting collections are counted and streamed from that temporary export index, avoiding repeated joins across the production `names`, `resource_summary`, `tlsa_evidence_summary`, and `provider_summary` tables.
