...
The scanner imports active, actionable roots with direct HNS nameserver IP evidence from SYNTH or GLUE. It also treats bounded shared delegation and HNS nameserver-handoff cohorts as first-tier website signals. A handoff cohort shares the same nameserver, HNS root, bootstrap address, and bootstrap field; the probe first resolves that HNS NS handoff, then queries the delegated zone. This captures managed namespace patterns without requiring duplicate parent-side GLUE, while avoiding the largest generic parking clusters. The scanner creates only the root apex candidate by default.

DS alone is not a website signal. There is one explicit exception for an indirect HNS nameserver handoff: every active, actionable DS-backed handoff is first resolved through the configured HNS DoH resolver. Only a public A/AAAA result carrying DNS `AD` (resolver-validated) is admitted to the detailed HTTP/HTTPS scanner. This resolver-first preflight is route-independent and therefore includes bounded cohorts, singletons, and large shared routes; it makes no target web request itself. A resolver-authenticated no-address result retries after 30 days; a resolver timeout or other unavailable response retries the next day. Successful resolver admissions are refreshed after seven days.
...
Two compact priority indexes are refreshed independently on `denuoweb-vm`: shared delegation groups from the published nameserver shards, and HNS nameserver-handoff routes from `data/hns-handoff-groups.json`. The same handoff artifact also carries the complete DS-backed resolver-preflight set. A handoff cohort groups roots by the same nameserver, HNS root, bootstrap address, and bootstrap field, retaining only groups with 2–250 members for direct broad web sweeps. Separately, DNSSEC-signed singleton HNS handoff routes are retained as priority routes. Each route larger than 250 roots contributes at most three deterministic DNSSEC-signed canaries for direct broad sweeps; its DS members are nevertheless all eligible for resolver-only AD preflight. This lets the scanner target both like-resource HNS delegations and DNSSEC-valid indirect routes such as `shakeshift` without treating a shared route by itself as evidence of a website. TLSA-unobserved remains a broad remediation queue rather than a high-confidence liveness signal.
...
hns-live-directory preflight-handoffs --db data/live.sqlite --limit 50
