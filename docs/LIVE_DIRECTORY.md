...
DS alone is not a website signal. There is one explicit exception for an indirect HNS nameserver handoff: every active, actionable DS-backed handoff is first resolved through the configured HNS DoH resolver. Only a public A/AAAA result carrying DNS `AD` (resolver-validated) is admitted to the detailed HTTP/HTTPS scanner. This resolver-first preflight is route-independent and therefore includes bounded cohorts, singletons, and large shared routes; it makes no target web request itself. Names without an AD-validated result remain out of the web queue and retry after 30 days. Successful resolver admissions are refreshed after seven days.
